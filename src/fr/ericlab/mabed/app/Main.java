...
import br.cos.ufrj.SplitTweets;
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length == 0 || args[0].equals("-help")) {
			...
			if (args[0].equals("-run")) {
				run(configuration, corpus);
			} else if (args[0].equals("-split") && args.length == 3) {
				SplitTweets.split(Integer.parseInt(args[2]), args[1]);
			} else {
	...
	private static void run(Configuration configuration, Corpus corpus) throws IOException {
		try {
			if (configuration.numberOfThreads > 1) {
				System.out.println("Running the parallelized implementation with " + configuration.numberOfThreads
						+ " threads (this computer has " + Runtime.getRuntime().availableProcessors() + " available threads)");
			} else {
			...
			output += "-------------------------\n" + Util.getDate() + " MABED is running\n-------------------------\n";
			System.out.println(Util.getDate() + " Reading parameters:\n   - k = " + configuration.k + ", p = " + configuration.p
					+ ", theta = " + configuration.theta + ", sigma = " + configuration.sigma);
